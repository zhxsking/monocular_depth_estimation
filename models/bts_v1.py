...
def set_misc(model, bn_no_track_stats=True, fix_first_conv_blocks=False, fix_first_conv_block=False):
...
class bts(nn.Module):
    def __init__(self, max_depth_, feat_out_channels, num_features=512):
        ...
        self.daspp_conv = torch.nn.Sequential(
            ...
            nn.ELU())
        self.reduc8x8 = reduction_1x1(num_features // 4, num_features // 4, self.max_depth_)
        ...
        self.conv3 = torch.nn.Sequential(
            ...
            nn.ELU())
        self.reduc4x4 = reduction_1x1(num_features // 4, num_features // 8, self.max_depth_)
        ...
        self.reduc2x2 = reduction_1x1(num_features // 8, num_features // 16, self.max_depth_)
        self.lpg2x2 = local_planar_guidance(2)

        self.upconv1 = upconv(num_features // 8, num_features // 16)
        self.reduc1x1 = reduction_1x1(num_features // 16, num_features // 32, self.max_depth_, is_final=True)
        self.conv1 = torch.nn.Sequential(nn.Conv2d(num_features // 16 + 5, num_features // 16, 3, 1, 1, bias=False),
                                         nn.ELU())
        self.get_depth = torch.nn.Sequential(nn.Conv2d(num_features // 16, 1, 3, 1, 1, bias=False),
                                             nn.Sigmoid())

        self.lidar1 = torch.nn.Conv2d(1, 1, bias=False, kernel_size=1, stride=1, padding=0)

    def forward(self, features, lidar):
        ...
        depth_8x8_scaled = depth_8x8.unsqueeze(1) / self.max_depth_
        ...
        depth_4x4_scaled = depth_4x4.unsqueeze(1) / self.max_depth_
        ...
        depth_2x2_scaled = depth_2x2.unsqueeze(1) / self.max_depth_

        upconv1 = self.upconv1(iconv2)
        reduc1x1 = self.reduc1x1(upconv1)

        l1 = self.lidar1(lidar / self.max_depth_)

        concat1 = torch.cat([l1, upconv1, reduc1x1, depth_2x2_scaled, depth_4x4_scaled, depth_8x8_scaled], dim=1)
        iconv1 = self.conv1(concat1)
        final_depth = self.max_depth_ * self.get_depth(iconv1)
        # if self.params.dataset == 'kitti':
        #     final_depth = final_depth * focal.view(-1, 1, 1, 1).float() / 715.0873
        output = [final_depth, depth_8x8_scaled, depth_4x4_scaled, depth_2x2_scaled, reduc1x1]
        return output


class encoder(nn.Module):
    def __init__(self, encoder_='densenet121_bts'):
        super().__init__()
        if encoder_ == 'densenet121_bts':
        ...
        elif encoder_ == 'densenet161_bts':
            self.base_model = models.densenet161(pretrained=True).features
            self.feat_names = ['relu0', 'pool0', 'transition1', 'transition2', 'norm5']
            self.feat_out_channels = [96, 96, 192, 384, 2208]
        elif encoder_ == 'resnet50_bts':
            self.base_model = models.resnet50(pretrained=True)
            self.feat_names = ['relu', 'layer1', 'layer2', 'layer3', 'layer4']
            self.feat_out_channels = [64, 256, 512, 1024, 2048]
        elif encoder_ == 'resnet101_bts':
            self.base_model = models.resnet101(pretrained=True)
            self.feat_names = ['relu', 'layer1', 'layer2', 'layer3', 'layer4']
            self.feat_out_channels = [64, 256, 512, 1024, 2048]
        elif encoder_ == 'resnext50_bts':
            self.base_model = models.resnext50_32x4d(pretrained=True)
            self.feat_names = ['relu', 'layer1', 'layer2', 'layer3', 'layer4']
            self.feat_out_channels = [64, 256, 512, 1024, 2048]
        elif encoder_ == 'resnext101_bts':
            self.base_model = models.resnext101_32x8d(pretrained=True)
            self.feat_names = ['relu', 'layer1', 'layer2', 'layer3', 'layer4']
            self.feat_out_channels = [64, 256, 512, 1024, 2048]
        else:
            print('Not supported encoder: {}'.format(encoder_))
...
class BtsModel_v1(nn.Module):
    def __init__(self, max_depth_, encoder_='densenet121_bts', bts_size=512):
        super().__init__()
        self.encoder_name = encoder_
        self.encoder = encoder(encoder_)
        self.decoder = bts(max_depth_, self.encoder.feat_out_channels, bts_size)
        self.decoder.apply(weights_init_xavier)

    def forward(self, input):
        x, lidar = input
        skip_feat = self.encoder(x)
        return self.decoder(skip_feat, lidar)


if __name__ == '__main__':
    H, W = 256, 512
    model = BtsModel_v1(10).cuda()
    print(model)
    print("Number of parameters in model is {:.3f}M".format(sum(tensor.numel() for tensor in model.parameters())/1e6))
    x = torch.rand((1, 3, H, W)).cuda().float()
    lidar = torch.rand((1, 1, H, W)).cuda().float()
    out = model([x, lidar])
    print(out[0].shape)
